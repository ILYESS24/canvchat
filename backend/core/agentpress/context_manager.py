...
# Mock token_counter - litellm not available
def token_counter(model: str, text: str) -> int:
    """Mock token counter - returns approximate token count"""
    # Rough approximation: 1 token ≈ 4 characters for most models
    return len(text) // 4
# Mock Anthropic - not available in deployment
class Anthropic:
...
class MockContent:
    def __init__(self):
        self.type = "text"
        self.text = "Mock Anthropic response - AI services not available"
...
def _get_bedrock_client_singleton():
    ...
    if _bedrock_client is None:
        try:
            # Mock boto3 - not available in deployment
            class MockBoto3Client:
                def invoke_model(self, **kwargs):
                    return MockBoto3Response()

            class MockBoto3Response:
                def __init__(self):
                    self['body'] = MockStreamingBody()

            class MockStreamingBody:
                def read(self):
                    return b'{"completion": "Mock AWS Bedrock response - AI services not available"}'

            _bedrock_client = MockBoto3Client()
