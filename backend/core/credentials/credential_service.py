...
class EncryptionService:
    ...
    def _get_or_create_encryption_key(self) -> bytes:
        ...
        mcp_key = os.getenv("MCP_CREDENTIAL_ENCRYPTION_KEY")
        enc_key = os.getenv("ENCRYPTION_KEY")
        key_env = mcp_key or enc_key

        env_mode = os.getenv("ENV_MODE", "LOCAL").upper()

        logger.debug(f"Encryption key check - ENV_MODE: {env_mode}, MCP_CREDENTIAL_ENCRYPTION_KEY: {'set' if mcp_key else 'not set'}, ENCRYPTION_KEY: {'set' if enc_key else 'not set'}")

        if not key_env:
            if env_mode == "LOCAL":
                logger.warning("No encryption key found in LOCAL environment, generating new key")
                # Generate a new Fernet key for development/testing
                new_key = Fernet.generate_key()
                logger.info(f"Generated new encryption key. Set ENCRYPTION_KEY={new_key.decode()} in your environment")
                return new_key
            else:
                # In PRODUCTION/STAGING, require encryption key to be set
                logger.error("❌ ENCRYPTION_KEY environment variable is required in production/staging")
                logger.error("Set ENCRYPTION_KEY to a valid Fernet key in your deployment environment")
                logger.error("Generate a key with: python3 -c \"from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())\"")
                raise ValueError("ENCRYPTION_KEY environment variable must be set in production/staging environments")
        ...
        try:
            ...
            logger.error("The ENCRYPTION_KEY must be a valid 32-byte base64-encoded Fernet key")
            ...
            logger.error("Check your Render environment variables - ENCRYPTION_KEY or MCP_CREDENTIAL_ENCRYPTION_KEY is set to an invalid value")
