...
# Mock novu_py - not available in deployment
logger.warning("novu_py not available, using mock implementation for deployment compatibility")

class MockTriggerEventRequestDto:
    def __init__(self, workflow_id: str, to: str, payload: Dict[str, Any]):
        self.workflow_id = workflow_id
        self.to = to
        self.payload = payload
        self.overrides = None

class MockCreateSubscriberRequestDto:
    def __init__(self, subscriber_id: str, email: Optional[str] = None, first_name: Optional[str] = None,
                 last_name: Optional[str] = None, phone: Optional[str] = None, avatar: Optional[str] = None,
...
class MockPatchSubscriberRequestDto:
    def __init__(self, email: Optional[str] = None, first_name: Optional[str] = None,
                 last_name: Optional[str] = None, phone: Optional[str] = None, avatar: Optional[str] = None,
                 data: Optional[Dict[str, Any]] = None):
        self.email = email
        self.first_name = first_name
...
class MockSubscribers:
    def create(self, create_subscriber_request_dto):
        logger.info("Mocking novu subscribers.create")

    def patch(self, subscriber_id: str, patch_subscriber_request_dto):
        logger.info(f"Mocking novu subscribers.patch for {subscriber_id}")

    def delete(self, subscriber_id: str):
        logger.info(f"Mocking novu subscribers.delete for {subscriber_id}")

    def retrieve(self, subscriber_id: str):
        logger.info(f"Mocking novu subscribers.retrieve for {subscriber_id}")
        return {}

    @property
    def preferences(self):
        return MockSubscriberPreferences()

class MockSubscriberPreferences:
    def update(self, subscriber_id: str, workflow_id: str, update_subscriber_preference_request_dto: Dict[str, Any]):
        logger.info(f"Mocking novu subscribers.preferences.update for {subscriber_id}, workflow {workflow_id}")

class MockNovu:
    def __init__(self, server_url: str, secret_key: str):
        self.server_url = server_url
        self.secret_key = secret_key
        logger.warning("Mocking Novu client. Notification services not available.")

    def trigger(self, trigger_event_request_dto):
        logger.info(f"Mocking novu trigger for workflow {trigger_event_request_dto.workflow_id}")
        return MockResponse()
...
novu_py = type('module', (), {
    'TriggerEventRequestDto': MockTriggerEventRequestDto,
    'CreateSubscriberRequestDto': MockCreateSubscriberRequestDto,
    'PatchSubscriberRequestDto': MockPatchSubscriberRequestDto,
})()
...
class NovuService:
    ...
    async def update_subscriber_credentials(
        ...
        if not self.enabled:
            logger.debug(f"Skipping credential update (Novu disabled in {config.ENV_MODE.value} mode)")
            return True
    ...
    async def register_push_token(
        ...
        if not self.enabled:
            logger.debug(f"Skipping push token registration (Novu disabled in {config.ENV_MODE.value} mode)")
            return True
