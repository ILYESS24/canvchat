...
const API_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'https://suna-backend.onrender.com';
...
async function makeRequest<T = any>(
  ...
  const isMockMode = false; // Always use real API routes now

  if (isMockMode) {
    console.log(`[MOCK] API call to ${url} - using mock response`);

    // Return mock responses based on URL patterns - more comprehensive matching
    if (url.includes('/agents') || url.includes('agents')) {
      // Handle agent-related endpoints
      if (url.includes('/generate-icon') || url.includes('generate-icon')) {
        return {
          data: { icon_name: 'default', icon_color: '#3b82f6', icon_background: '#1e40af' },
          success: true
        };
      }
      return {
        data: {
          agents: [],
          total: 0,
          message: 'Backend not yet deployed. Please deploy the Python backend separately.'
        },
        success: true
      };
    }
    ...
    if (url.includes('/threads') || url.includes('threads')) {
      // Handle thread-related endpoints
      return {
        data: {
          threads: [],
          total: 0,
    ...
    if (url.includes('/messages') || url.includes('messages')) {
      // Handle message-related endpoints
      return {
        data: {
          messages: [],
          total: 0,
    ...
    // Default mock response for any other API call
    return {
      data: {
        message: 'Backend not yet deployed. Please deploy the Python backend separately.',
        mock: true,
