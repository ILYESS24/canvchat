...
// Temporary types (simplified to avoid TypeScript errors)
type ToolCallAccumulatorState = any;
type UnifiedMessage = any;
type ToolCallStreamMessage = any;
type ToolOutputMessage = any;

// Temporary functions (simplified implementations)
const createAccumulatorState = () => ({ toolCalls: [], pendingDeltas: [], completedToolCalls: [] });
const clearAccumulator = () => {};
const accumulateToolCallDeltas = () => {};
const reconstructToolCalls = () => [];
const markToolCallCompleted = () => {};
