...
manual_instructions() {
    echo -e "${BLUE}📋 Manual Render Deployment Instructions:${NC}"
    echo ""
    echo "Since Render CLI is not available, please deploy manually:"
    echo ""
    echo "1. ${YELLOW}Connect your repository${NC}:"
    echo "   - Go to https://render.com"
    echo "   - Connect your GitHub repository"
    echo "   - Select the kortix repository"
    echo ""
    ...
    echo "   - PostgreSQL: Use Render PostgreSQL"
    echo "   - Redis: Use Render Redis"
    echo ""
    echo "5. ${YELLOW}Set Environment Variables${NC}:"
    echo "   Copy variables from render-env-example.txt to each service"
    echo ""
    echo "6. ${YELLOW}Update Frontend URLs${NC}:"
    echo "   Set NEXT_PUBLIC_BACKEND_URL to your backend service URL"
    echo ""
}
